...
from app.api.deps import (
    ...
    get_tiles_service,
    get_tiles_cache,
    TILES_BACKEND, TILES_BUCKET, TILES_FS_DIR,
)
from app.services.original_image_service import OriginalImageService
...
def cache_stats(
    ...
    stats = cache.stats()
    storage = {"bucket": TILES_BUCKET} if TILES_BACKEND == "s3" else {"directory": TILES_FS_DIR}
...
def get_tile(
    ...
    """
    Доступ к тайлу (z, y, x) + кеширование.
    """
    try:
...
def admin_delete_all_tiles_global(
    tiles: TilesService = Depends(get_tiles_service),
):
    try:
...
@tiles_router.delete("/_cache", response_model=ClearCacheResponse)
def clear_cache(
    cache: InMemoryTTLCache[object, bytes] = Depends(get_tiles_cache),
):
    cache.clear()
    return ClearCacheResponse(message="cache cleared")
